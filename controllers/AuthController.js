    ...
    async login(req, res) { //pode logar com EMAIL ou CPF
        try {
            ...
            if (emailRegex.test(emailOrCpf)) {
                member = await prismaClient.membrosManancialQualificacoes.findUnique({where: {email: emailOrCpf}});
            } else if (cpfRegex.test(emailOrCpf)) {
                member = await prismaClient.membrosManancialQualificacoes.findUnique({where: {cpf: emailOrCpf}});
            } else {
            ...
            const passwordMatch = await bcrypt.compare(password, member.senha);
            if (!passwordMatch) return res.status(400).json({message: "Senha incorreta"});
            const jwToken = jwt.sign({id_membro: member.id_membro}, process.env.JWT_SECRET, {expiresIn: "1y"});
